...
[OpenDroneMap simple job](../../job_bundles/opendronemap_simple_job/) and the
proposed
[OpenDroneMap parallel job](../../job_bundles/opendronemap_parallel_job/).
...
2. Create a service-managed fleet using Linux and `x86_64`. Set its minimum
   worker count to zero and choose a bounded maximum worker count. Allow at
   least 2 vCPUs and 8 GiB RAM for the simple ODM job.
3. Add a custom fleet attribute named `attr.OpenDroneMap` with the value
   `docker_3_6_1`. The ODM templates require this capability, so they cannot
   run on a Linux fleet where this configuration was not applied.
   For the parallel ODM sample, also add a custom fleet amount named
   `amount.OpenDroneMapScratchGiB`; set its minimum and maximum to the root
   volume size in GiB so Deadline can enforce the sample's scratch estimate.
...
Configuration changes apply only to workers launched after the update. Set the
maximum worker count to at least two when testing the parallel sample so its
fan-out can use more than one worker. Parallel-job CPU, memory, and scratch
minimums remain provisional until its benchmark plan is complete.
...
The fleet must already have the `attr.OpenDroneMap=docker_3_6_1` custom
attribute. Parallel fleets also need `amount.OpenDroneMapScratchGiB` as
described above. Scale existing workers to zero before launching a replacement
worker, because an in-service worker does not rerun the updated host
configuration.
